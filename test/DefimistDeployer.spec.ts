...
describe('DefimistDeployer', () => {
  const provider = new MockProvider({
    hardfork: 'istanbul',
    mnemonic: 'horn horn horn horn horn horn horn horn horn horn horn horn',
    gasLimit: 12000000
  })
  const [dmdao, tokenOwner, protocolFeeReceiver, other] = provider.getWallets()
  const overrides = {
    gasLimit: 12000000
  }

  let defimistDeployer: Contract
  let token0: Contract
  let token1: Contract
  let token2: Contract
  const pairBytecode = "0x"+DefimistPair.bytecode
  ...
  it('Execute migration with intial pairs', async () => {
    ...
    // Deploy DefimistDeployer
    defimistDeployer = await deployContract(
    ...
    expect(await defimistDeployer.state()).to.eq(0)
    
    // Dont allow other address to approve deployment by sending eth
    await expect(other.sendTransaction({to: defimistDeployer.address, gasPrice: 0, value: expandTo18Decimals(10000)}))
      .to.be.revertedWith('DefimistDeployer: CALLER_NOT_FEE_TO_SETTER')
    
    // Dont allow deploy before being approved by sending ETH
    await expect(defimistDeployer.connect(other).deploy())
      .to.be.revertedWith('DefimistDeployer: WRONG_DEPLOYER_STATE')

    // Send transaction with value from dmdao to approve deployment
    await dmdao.sendTransaction({to: defimistDeployer.address, gasPrice: 0, value: expandTo18Decimals(10000)})
    expect(await defimistDeployer.state()).to.eq(1)
    
    // Dont allow sending more value
    await expect(dmdao.sendTransaction({to: defimistDeployer.address, gasPrice: 0, value: expandTo18Decimals(10000)}))
      .to.be.revertedWith('DefimistDeployer: WRONG_DEPLOYER_STATE')
    await expect(other.sendTransaction({to: defimistDeployer.address, gasPrice: 0, value: expandTo18Decimals(10000)}))
      .to.be.revertedWith('DefimistDeployer: WRONG_DEPLOYER_STATE')

    // Execute deployment transaction
    const deployTx = await defimistDeployer.connect(other).deploy()
    expect(await defimistDeployer.state()).to.eq(2)
    ...
    await expect(defimistDeployer.connect(other).deploy()).to.be.revertedWith('DefimistDeployer: WRONG_DEPLOYER_STATE')
    ...
    const pairFactory = new Contract(pairFactoryAddress, JSON.stringify(DefimistFactory.abi), provider);
    const pair01 = new Contract(
      getCreate2Address(pairFactory.address, [token0.address, token1.address], pairBytecode), 
      JSON.stringify(DefimistPair.abi),
      provider
    );
