...
describe('DefimistFeeReceiver', () => {
  ...
  const [dmdao, wallet, protocolFeeReceiver, other] = provider.getWallets()
  const loadFixture = createFixtureLoader(provider, [dmdao, wallet, protocolFeeReceiver])
    ...
    async () => 
  {
    ...
    expect((await token0.balanceOf(dmdao.address)))
  ...
  it('should receive everything in ETH from one WETH-token1 pair', async () => {
    ...
    expect(await provider.getBalance(feeReceiver.address)).to.eq(0)
    expect(await token1.balanceOf(dmdao.address)).to.be.eq(0)
    expect((await provider.getBalance(protocolFeeReceiver.address)))
    ...
    async () => 
  {
    ...
    expect((await tokenA.balanceOf(dmdao.address)))
      .to.be.eq(tokenAFromProtocolFee)
    expect((await tokenB.balanceOf(dmdao.address)))
    ...
    async () =>
  {
    ...
    expect((await tokenB.balanceOf(dmdao.address)))
      .to.be.eq(tokenBFromProtocolFee)
    expect((await tokenC.balanceOf(dmdao.address)))
      .to.be.eq(tokenCFromProtocolFee)
    expect((await tokenD.balanceOf(dmdao.address)))
    ...
    async () =>
  {
    await expect(feeReceiver.connect(other).transferOwnership(other.address))
      .to.be.revertedWith('DefimistFeeReceiver: FORBIDDEN')
    await feeReceiver.connect(dmdao).transferOwnership(other.address);
    ...
    async () =>
  {
    await expect(feeReceiver.connect(other).changeReceivers(other.address, other.address))
      .to.be.revertedWith('DefimistFeeReceiver: FORBIDDEN')
    await feeReceiver.connect(dmdao).changeReceivers(other.address, other.address);
    ...
    async () => 
  {
    ...
    await expect(feeReceiver.connect(wallet).takeProtocolFee([pair.address], overrides)).to.be.revertedWith('DefimistFeeReceiver: INSUFFICIENT_LIQUIDITY')
    ...
    expect((await token0.balanceOf(dmdao.address)))
      .to.be.eq(0)
    expect((await token1.balanceOf(dmdao.address)))
      .to.be.eq(0)
  })
})
