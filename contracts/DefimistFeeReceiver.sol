...
contract DefimistFeeReceiver {
    ...
    function transferOwnership(address newOwner) external {
        require(msg.sender == owner, 'DefimistFeeReceiver: FORBIDDEN');
        owner = newOwner;
    }
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'DefimistFeeReceiver: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'DefimistFeeReceiver: ZERO_ADDRESS');
    }
    ...
    // Taken from DefimistLibrary, removed the factory parameter
    ...
    // Done with code form DefimistRouter and DefimistLibrary, removed the deadline argument
    function _swapTokensForETH(uint amountIn, address fromToken)
        internal
    {
        IDefimistPair pairToUse = IDefimistPair(pairFor(fromToken, WETH));
        ...
        require(reserveIn > 0 && reserveOut > 0, 'DefimistFeeReceiver: INSUFFICIENT_LIQUIDITY');
    ...
    // Take what was charged as protocol fee from the Defimist pair liquidity
    function takeProtocolFee(IDefimistPair[] calldata pairs) external {
